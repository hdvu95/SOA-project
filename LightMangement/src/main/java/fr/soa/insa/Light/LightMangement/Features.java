package fr.soa.insa.Light.LightMangement;
...
import fr.soa.insa.Light.LightMangement.model.ArchiveActions;
import fr.soa.insa.Light.LightMangement.model.Config;
...
public class Features {
	...
	public int turnOff(@PathVariable("dept") String dept, @PathVariable("room") String room) throws IOException{
		int ans = HTTPManagement.sendPOST("http://"+ipAddr+":"+port+"/~/mn-cse-light/mn-light/"+dept+"/"+room, "0");
		if (ans == 201){
	...
	public int turnOn(@PathVariable("dept") String dept, @PathVariable("room") String room) throws IOException{
		int ans = HTTPManagement.sendPOST("http://"+ipAddr+":"+port+"/~/mn-cse-light/mn-light/"+dept+"/"+room, "1");
		if(ans == 201){
			ArchiveActions aa = new ArchiveActions("ON",HTTPManagement.getDate(), dept, room);
			archive.add(aa);
		}
		return ans;
	}
	...
	public String getStatus(@PathVariable("dept") String dept,@PathVariable("room") String room) throws IOException{
		String status = "-1";
		String ans = HTTPManagement.sendGET("http://"+ipAddr+":"+port+"/~/mn-cse-light/mn-light/"+dept+"/"+room+"/la");
