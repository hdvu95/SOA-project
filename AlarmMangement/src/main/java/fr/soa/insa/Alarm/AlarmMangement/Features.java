...
import fr.soa.insa.Alarm.AlarmMangement.model.Config;
...
public class Features {
	...
	public int turnOff(@PathVariable("dept") String dept, @PathVariable("room") String room) throws IOException{
		int ans = HTTPManagement.sendPOST("http://"+ipAddr+":"+port+"/~/mn-cse-alarm/mn-alarm/"+dept+"/"+room, "0");
		if(ans == 201){
			ArchiveActions aa = new ArchiveActions("turnOff",HTTPManagement.getDate(),dept, room);
			archive.add(aa);
		}
		return ans;
	}
	...
	public int turnOn(@PathVariable("dept") String dept, @PathVariable("room") String room) throws IOException{
		int ans = HTTPManagement.sendPOST("http://"+ipAddr+":"+port+"/~/mn-cse-alarm/mn-alarm/"+dept+"/"+room, "1");
		if(ans == 201){
			ArchiveActions aa = new ArchiveActions("turnOn",HTTPManagement.getDate(), dept, room);
			archive.add(aa);
		}
		return ans;
	}
	...
	public String getStatus(@PathVariable("dept") String dept,@PathVariable("room") String room) throws IOException{
		String status = "-1";
		String ans = HTTPManagement.sendGET("http://"+ipAddr+":"+port+"/~/mn-cse-alarm/mn-alarm/"+dept+"/"+room+"/la");
