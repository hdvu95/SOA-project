package fr.soa.insa.Windows.WindowsMangement;
...
import fr.soa.insa.Windows.WindowsMangement.model.ArchiveActions;
import fr.soa.insa.Windows.WindowsMangement.model.Config;
...
public class Features {
	...
	public int turnOff(@PathVariable("dept") String dept, @PathVariable("room") String room) throws IOException{
		int ans = HTTPManagement.sendPOST("http://"+ipAddr+":"+port+"/~/mn-cse-windows/mn-windows/"+dept+"/"+room, "0");
		if (ans == 201){
	...
	public int turnOn(@PathVariable("dept") String dept, @PathVariable("room") String room) throws IOException{
		int ans = HTTPManagement.sendPOST("http://"+ipAddr+":"+port+"/~/mn-cse-windows/mn-windows/"+dept+"/"+room, "1");
		if(ans == 201){
	...
	public String getStatus(@PathVariable("dept") String dept,@PathVariable("room") String room) throws IOException{
		String status = "-1";
		String ans = HTTPManagement.sendGET("http://"+ipAddr+":"+port+"/~/mn-cse-windows/mn-windows/"+dept+"/"+room+"/la");
